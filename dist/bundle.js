...
/***/ function(module, exports, __webpack_require__) {
	...
	function main(sources) {
		var moveLeft$ = sources.DOM.select('.move-left').events('click').mapTo(-1);

		var moveRigth$ = sources.DOM.select('.move-rigth').events('click').mapTo(+1);

		var state$ = _xstream2.default.merge(moveLeft$, moveRigth$);

		var sinks = {
			DOM: state$.fold(function (acc, x) {
				return acc + x;
			}, 20).map(function (value) {
				return (0, _dom.div)([(0, _dom.button)('.move-left', 'move left'), (0, _dom.input)({ attrs: { type: 'range', min: 0, max: 100, value: value } }), (0, _dom.button)('.move-rigth', 'move rigth')]);
			})
		};
